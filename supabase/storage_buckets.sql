...
create policy book_covers_write on storage.objects for all to authenticated
  using (bucket_id = 'book_covers') with check (bucket_id = 'book_covers');
drop policy if exists store_photos_write on storage.objects;
create policy store_photos_write on storage.objects for all to authenticated
...
create policy author_photos_write on storage.objects for all to authenticated
